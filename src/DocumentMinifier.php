 ...
 * `deminifyDocument` will delete the entire `tempDir` with `cleanup` set to `true` (enabled
...
class DocumentMinifier
{
    ...
    public function minifyDocument(string $inputFilePath, $cleanup = false): string
    {
        ...
        if ($cleanup) {
            $this->recursivelyDeleteDirectory($extractedDocDirectory);
        }
    ...
    private function exportMediaToMediaDirAndReplace(string $inputDirectory, string $mediaDirectory)
    {
        ...
        foreach ($iterator as $file) {
            if ($file->isFile() && array_key_exists($file->getExtension(), DocumentMinifier::SUPPORTED_MEDIA_FORMATS)) {
                ...
                // This replaces any supported media with a 1-pixel PNG.
                // This works for images, videos and GIFs (despite the media type mismatch in some cases)
                // TBD if this would work for audio files
    ...
    private function recursivelyDeleteDirectory($dir)
    {
        if (is_dir($dir)) {
            $objects = scandir($dir);
            foreach ($objects as $object) {
                if ($object !== '.' && $object !== '..') {
                    if (filetype($dir . '/' . $object) === 'dir') {
                        DocumentMinifier::recursivelyDeleteDirectory($dir . '/' . $object);
                    } else {
                        unlink($dir . '/' . $object);
                    }
                }
